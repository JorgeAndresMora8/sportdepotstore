...
function Signup() {
  ...
  const handleSubmit = async (event: any) => {
    ...
    try {
      ...
      if(!resp.name){ 
        throw new Error(`there was an error ${JSON.stringify(resp)}`)
      }
    ...
    <div className={Styles.container}>
      ...
      <div className={Styles.form__container}>
        ...
        <Form onSubmit={handleSubmit}>
          ...
          { loading ? <button>loading</button> : <Form.SubmitButton>Crear Perfil</Form.SubmitButton>}
        </Form>
        <p>
          ¿Ya tienes una cuenta? <Link to={"/login"}>Accede aqui</Link>
        </p>
        {error && <p>Hubo un error al crear el usuario</p>}
      </div>
    </div>
  );
}
